...
def data_generator(wave_path, buffers_per_second=10):
    ...
    with wave.open(wave_path, 'rb') as file:
        ...
        while frames:
            frames, resampler_state = _resample_frames(frames, channel_count, sample_rate,
                                                       resampler_state)
            yield frames
            frames = file.readframes(frames_per_buffer)
...
def _resample_frames(frames, channel_count, sample_rate, resampler_state=None):
    ...
    if sample_rate != TARGET_SAMPLE_RATE:
        frames, resampler_state = audioop.ratecv(frames, SUPPORTED_SAMPLE_WIDTH,
                                                 TARGET_CHANNEL_COUNT, sample_rate,
                                                 TARGET_SAMPLE_RATE, resampler_state)
